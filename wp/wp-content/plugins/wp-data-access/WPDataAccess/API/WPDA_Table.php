...
class WPDA_Table extends WPDA_API_Core {
    ...
    public function select(
        ...
        if ( null === $wpdadb ) {
                ...
                foreach ( $column_names as $column_name => $queryable ) {
                    if ( $queryable ) {
                        if ( isset( $lookups[$column_name] ) ) {
                            ...
                            $where_global[] = $wpdadb->prepare( " `%1s` like '%s' ", array($column_name, '%' . esc_sql( $search ) . '%') );
                        }
                    }
                }
            }
            ...
            // Perform query.
            $dataset = $wpdadb->get_results( $wpdadb->prepare( $sql, array($tbl) ), 'ARRAY_A' );
            if ( $wpdadb->last_error ) {
                // Handle SQL errors.
                return new \WP_Error('error', $wpdadb->last_error, array(
                    'status' => 420,
    ...
    private function add_condition( $where_lines, $operand = 'and' ) {
